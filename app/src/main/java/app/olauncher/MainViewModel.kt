...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun selectedApp(appModel: AppModel, flag: Int) {
        when (flag) {
            ...
            Constants.FLAG_SET_HOME_APP(apNumber:int) -> {
                //suppose to work after fixing the getter and setter problem in pefs.kt
                prefs.appNames[appNumber] = appModel.appLabel
                prefs.appPackages[appNumber] = appModel.appPackage
                prefs.appUsers[appNumber] = appModel.user.toString()
                prefs.appActivityClassNames[appNumber] = appModel.activityClassName
                refreshHome(false)
            }
